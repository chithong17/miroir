  ...
  return <section className="grid gap-5">
    ...
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      {plans.map((plan) => {
        ...
        return <article key={plan.code} className="flex flex-col rounded-2xl border border-[#DFE8D5] bg-white p-5">
          ...
          <div className="mt-3 flex items-center gap-2">
            <p className="text-sm text-slate-700">Phí năm: <strong>{money(annualPrice)}</strong></p>
            {annualDiscount > 0 ? <span className="inline-flex items-center rounded-md bg-red-50 px-2 py-0.5 text-[10px] font-bold text-red-700 ring-1 ring-inset ring-red-600/10">Tiết kiệm {annualDiscount}%</span> : null}
          </div>

...
import {
  ...
  createRefundDispute,
  createReturnRequest, escalateReturn, listMyDisputes, listMyReturns, replyMyDispute, submitReturnShipment, submitFitFeedback,
...
function FitFeedbackCard({ order, setNotice }) {
  ...
  const submit = async (item) => {
    ...
    if (!outcome) return setNotice("Hãy chọn độ vừa vặn trước khi gửi phản hồi.");
    setBusy(item.variantId);
    try { await submitFitFeedback(order.id, item.variantId, outcome); setNotice("Cảm ơn bạn đã phản hồi về độ vừa vặn."); }
    catch (error) { setNotice(error.response?.data?.message || "Không thể gửi phản hồi Fit Finder."); }
    finally { setBusy(""); }
  };
  return <section className="miroir-card"><h2 className="text-xl font-black">Độ vừa vặn</h2><p className="mt-1 text-sm text-muted">Phản hồi này chỉ dùng ở dạng tổng hợp để shop cải thiện bảng size.</p><div className="mt-4 grid gap-3">{order.items.map((item) => <div className="rounded-xl bg-panel p-3" key={item.variantId}><p className="font-bold">{item.name} · {item.size || "Một cỡ"}</p><div className="mt-2 flex flex-wrap items-center gap-2"><select className="miroir-field !w-auto" value={outcomes[item.variantId] || ""} onChange={(event) => setOutcomes((current) => ({ ...current, [item.variantId]: event.target.value }))}><option value="">Chọn phản hồi</option><option value="too_small">Hơi chật / nhỏ</option><option value="true_to_size">Vừa đúng</option><option value="too_large">Hơi rộng / lớn</option></select><Button variant="secondary" disabled={busy === item.variantId} onClick={() => submit(item)}>{busy === item.variantId ? "Đang gửi..." : "Gửi"}</Button></div></div>)}</div></section>;
}
...
function ReturnRequests({ order, returns, onChanged, setNotice }) {
  const [open, setOpen] = useState(false); const [reason, setReason] = useState(""); const [reasonCode, setReasonCode] = useState("other"); const [bank, setBank] = useState({ bankName: "", accountNumber: "", accountHolder: "" }); const [quantities, setQuantities] = useState({}); const [images, setImages] = useState([]); const [busy, setBusy] = useState(false); const [formError, setFormError] = useState("");

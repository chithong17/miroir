    ...
    <main className="min-h-screen bg-white text-ink">
      <div className="grid min-h-screen lg:grid-cols-[248px_1fr]">
        ...
        <section className="p-5 lg:p-8">
          ...
          {view === "disputes" ? <AdminDisputesView disputes={disputes} notifications={adminNotifications} onRead={async (item) => { if (!item.readAt) { await readAdminNotification(item.id); setAdminUnread((count) => Math.max(count - 1, 0)); setAdminNotifications((all) => all.map((entry) => entry.id === item.id ? { ...entry, readAt: new Date() } : entry)); } }} onUpdate={async (id, status, resolution) => { try { await updateAdminDispute(id, { status, resolution }); await loadDisputes(); showNotice("Dispute updated."); } catch (error) { showNotice(error.response?.data?.message || "Could not update dispute.", "error"); } }} onReturnDecision={async (returnId, approved, reason, instructions) => { try { await decideAdminReturn(returnId, { approved, reason, instructions }); await loadDisputes(); showNotice("Return request updated."); } catch (error) { showNotice(error.response?.data?.message || "Could not update return request.", "error"); } }} /> : null}

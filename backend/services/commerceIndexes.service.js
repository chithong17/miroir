...
export const ensureCommerceIndexes = async () => {
  ...
  await Promise.all([
    ...
    db.collection("payment_orders").createIndex({ orderCode: 1 }, { unique: true }),
    db.collection("payment_orders").createIndex({ checkoutKey: 1 }, { unique: true, partialFilterExpression: { status: "pending", checkoutKey: { $type: "string" } } }),
    db.collection("billing_cycles").createIndex({ id: 1 }, { unique: true }),
    db.collection("billing_cycles").createIndex({ status: 1, expiresAt: 1 }),
    db.collection("billing_entries").createIndex({ id: 1 }, { unique: true }),
    db.collection("billing_entries").createIndex({ ownerId: 1, invoiceId: 1, createdAt: 1 }),
    db.collection("billing_invoices").createIndex({ id: 1 }, { unique: true }),
    db.collection("billing_invoices").createIndex({ status: 1, dueAt: 1 }),
    db.collection("billing_tryon_tasks").createIndex({ taskId: 1 }, { unique: true }),
    db.collection("order_cost_snapshots").createIndex({ orderId: 1 }, { unique: true }),
    db.collection("shop_advice").createIndex({ key: 1 }, { unique: true }),
    db.collection("shop_strategy_reports").createIndex({ ownerId: 1, cycleId: 1 }, { unique: true }),
